...
2. Copy **template** từ `references/doc-templates-library.md`
...
# Copy references/github-actions-docs.yml → .github/workflows/docs.yml
...
cp -r skills/templates/ /new-project/docs-skills/

# Copy configs
cp references/mkdocs-starter.yml /new-project/mkdocs.yml
cp references/markdownlint-config.json /new-project/.markdownlint.json
cp references/pre-commit-config.yaml /new-project/.pre-commit-config.yaml
```
...
cp references/github-actions-docs.yml .github/workflows/docs.yml
```
...
> 💡 **Team infra/ops:** Xem `references/infra-knowledge-base.md` để có starter structure sẵn cho network, Proxmox, Google Workspace, automation, GitHub, incidents.
